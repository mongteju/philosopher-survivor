...
// 1. Insert helper classes (Candlestick, RhythmicGridLine, NietzscheRelic) right before "class Boss"
const helperClasses = `
...
if (!code.includes('class Candlestick')) {
  code = code.replace('// ─── BOSS ───────────────────────────────────────────────────────────\r\nclass Boss {', helperClasses + '\r\n// ─── BOSS ───────────────────────────────────────────────────────────\r\nclass Boss {');
  code = code.replace('// ─── BOSS ───────────────────────────────────────────────────────────\nclass Boss {', helperClasses + '\n// ─── BOSS ───────────────────────────────────────────────────────────\nclass Boss {');
}

// 2. Overhaul class Boss implementation
const bossClassDefinition = `class Boss {
...
const bossTarget = code.match(/class Boss\s*\{[\s\S]*?\}\r?\n\}/)[0];
code = code.replace(bossTarget, bossClassDefinition);

// 3. Fix clone trigger in dealDamageToEnemy and onBossDefeated checks
// In dealDamageToEnemy, clones take 0/Miss damage:
const dealDmgTarget = `dealDamageToEnemy(e, dmg, proj) {`;
const dealDmgReplacement = `dealDamageToEnemy(e, dmg, proj) {
    if (e.isClone) {
      this.addDamageText(e.x, e.y - e.size - 10, "Miss (궤변)", "#7f8c8d", 14, false);
      return;
    }
`;
if (!code.includes('e.isClone) {')) {
  code = code.replace(dealDmgTarget, dealDmgReplacement);
}

// In remove dead enemies, filter clones from boss defeat stage trigger:
const filterDeadTarget = `        if (e.type === 'boss') {
          this.onBossDefeated(e);`;
const filterDeadReplacement = `        if (e.type === 'boss') {
          if (e.isClone) {
...
`;
code = code.replace(filterDeadTarget, filterDeadReplacement);

// 4. Recalculate stats for Ubermensch 각성 boost:
const recalculateTarget = `    // Recalculate aura bonuses`;
const recalculateReplacement = `    if (window.gameInstance && window.gameInstance.uberMenschMode) {
      this.dmgMultiplier *= 5;
    }

    // Recalculate aura bonuses`;
if (!code.includes('window.gameInstance.uberMenschMode')) {
  code = code.replace(recalculateTarget, recalculateReplacement);
}

// 5. Game.update(dt) updates for new grids, relics, candlesticks, and Wave 2 glitched HUD
const updateTimerTarget = `    // XP frags
    this.xpFrags.forEach(f => f.update(dt, this.player));`;
const updateTimerReplacement = `    // Update candlesticks
    ...
    // XP frags
    this.xpFrags.forEach(f => f.update(dt, this.player));`;
if (!code.includes('this.nietzcheRelics')) {
  code = code.replace(updateTimerTarget, updateTimerReplacement);
}

// 6. Game constructor helper declarations:
const constructorTarget = `    this.scroll = 0;
    window.gameInstance = this;`;
const constructorReplacement = `    this.scroll = 0;
    window.gameInstance = this;
...
`;
if (!code.includes('this.showBossTooltip = function')) {
  code = code.replace(constructorTarget, constructorReplacement);
}

// 7. Render custom pattern graphics inside Game.draw()
const drawZoneTarget = `    // Ataraxia safe zone
...
const drawZoneReplacement = `    // Ataraxia safe zone
    if (this.ataraxiaZone) {
      const rx = this.ataraxiaZone.x - camX + W / 2, ry = this.ataraxiaZone.y - camY + H / 2;
...
code = code.replace(drawZoneTarget, drawZoneReplacement);

// Render Candlesticks, Nietzsche Relics, and Grid Lines inside Game.draw()
const drawWarningTarget = `    // Warning zones
    this.warningZones.forEach(w => w.draw(ctx, this.camera));`;
const drawWarningReplacement = `    // Warning zones
    this.warningZones.forEach(w => w.draw(ctx, this.camera));
...
`;
if (!code.includes('// Draw Nietzsche relics')) {
  code = code.replace(drawWarningTarget, drawWarningReplacement);
}

// Stage 6 Grayscale and Wave 3 Sepia/Contrast filters inside Game.draw()
const drawSaveTarget = `    ctx.save();
    if (this.screenShake > 0) {`;
const drawSaveReplacement = `    ctx.save();
    if (this.screenShake > 0) {
...
`;
if (!code.includes('// Full grayscale filter for Stage 6')) {
...
// 8. Recalculate safe zone player BGM silencing in loop:
const updateBgmTarget = `    // Update HUD timer`;
const updateBgmReplacement = `    // Ataraxia safe zone BGM volume drop
    if (this.ataraxiaZone) {
    ...
    // Update HUD timer`;
if (!code.includes('Ataraxia safe zone BGM volume drop')) {
  code = code.replace(updateBgmTarget, updateBgmReplacement);
}

// 9. Recalculate Player Draw concentric active Auras and Glowing red fire ring:
const drawAuraTarget = `    // 등급별 특수 아우라 그리기`;
const drawAuraReplacement = `    // ─── Drawing active auras at player's feet ─────────────────────────────
    if (this.auras) {
    ...
    // 등급별 특수 아우라 그리기`;
if (!code.includes("Drawing active auras at player's feet")) {
  code = code.replace(drawAuraTarget, drawAuraReplacement);
}

// 10. Global Close Key Interception for Learned Skills Popup and Keyboard Aura choice navigation:
const globalKeydownTarget = `      // Level up card selection
      const lvlScreen = document.getElementById('levelup-screen');`;
const globalKeydownReplacement = `      // Global Learned Skills popup close check (Space / Escape)
      ...
      const lvlScreen = document.getElementById('levelup-screen');`;
if (!code.includes('Global Learned Skills popup close check')) {
  code = code.replace(globalKeydownTarget, globalKeydownReplacement);
}

const gachaKeyboardTarget = `      // Gacha screen
...
const gachaKeyboardReplacement = `      // Gacha screen
      const gachaScreen = document.getElementById('gacha-screen');
      if (gachaScreen && gachaScreen.classList.contains('active')) {
...
code = code.replace(gachaKeyboardTarget, gachaKeyboardReplacement);

// 11. Rename "은총의 불기둥" description references if any are still in the codebase or databases:
// Double check the rename:
code = code.replace(/'은총의 불기둥'/g, "'은총의 스파크'");
code = code.replace(/"은총의 불기둥"/g, '"은총의 스파크"');

fs.writeFileSync(filePath, code, 'utf8');
console.log("Successfully patched game.js with premium boss raid patterns, visual vector auras, and global close key intercepts!");
